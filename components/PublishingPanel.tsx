    ...
    <div className="space-y-6 relative">
      <div className="flex justify-between items-center">
        <div>
           ...
           <p className="text-slate-400">Gestión de publicación y visibilidad de calificaciones.</p>
        </div>
        <div className="text-right">
            <div className="text-sm font-medium text-slate-400">Estado Actual de la Vista</div>
            <div className="flex gap-4 mt-1">
                <span className="text-green-400 font-bold">{publishedCount} Publicadas</span>
                <span className="text-amber-400 font-bold">{pendingCount} Pendientes</span>
            </div>
        </div>
      </div>

      {/* Filters */}
      <div className="bg-[#1e293b] p-5 rounded-xl shadow-sm border border-slate-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <select 
                className="p-2 bg-[#0f172a] border border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-white"
                value={level}
             ...
             <div className="flex bg-[#0f172a] rounded-lg p-1 border border-slate-700">
                {[1, 2, 3].map((l) => (
                    <button
                        key={l}
                        onClick={() => setPeriod(l)}
                        className={`flex-1 py-1 text-sm font-medium rounded transition ${period === l ? 'bg-[#1e293b] shadow text-blue-400' : 'text-slate-400 hover:text-slate-200'}`}
                    >
                        Lapso {l}
                    </button>
                ))}
      ...
      {/* Action Bar */}
      <div className="flex justify-between items-center bg-blue-900/20 p-4 rounded-lg border border-blue-900/50">
          ...
          <button 
            ...
            className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium flex items-center gap-2"
          >
         ...
         {filteredRecords.length === 0 ? (
             ...
             <div className="overflow-x-auto">
                 <table className="w-full text-sm text-left">
                     <thead className="bg-[#0f172a] border-b border-slate-700 text-slate-400 uppercase font-bold text-xs">
                         <tr>
                     ...
                     <tbody className="divide-y divide-slate-700">
                         {filteredRecords.map((item) => (
                             <tr key={`${item.student.id}-${item.subject.id}`} className="hover:bg-[#2d3748]/30 transition">
                                 <td className="px-6 py-4 font-medium text-slate-200">
                                     {item.student.name}
                                     <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-[#0f172a] text-slate-400">
                                         {item.student.section}
                                     </span>
                                 </td>
                                 <td className="px-6 py-4 text-slate-300">{item.subject.name}</td>
                                 <td className="px-6 py-4 font-mono font-bold text-white">{item.gradeRec?.score}</td>
                                 <td className="px-6 py-4">
                                     <button 
                                        ...
                                        className={`flex items-center gap-2 max-w-[150px] truncate text-xs px-2 py-1 rounded transition ${
                                            ...
                                            ? 'bg-blue-900/50 text-blue-200 border border-blue-900/50' 
                                 ...
                                 <td className="px-6 py-4 text-center">
                                     {item.gradeRec?.isPublished ? (
                                         <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-green-900/50 text-green-200 text-xs font-semibold">
                                             <Eye className="w-3 h-3" /> Publicado
                                         </span>
                                     ) : (
                                         <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-slate-700 text-slate-300 text-xs font-semibold">
                                             Oculto
                                         </span>
                                     )}
                                 </td>
                                 <td className="px-6 py-4 text-right">
                                     <button
                                        ...
                                        className={`px-3 py-1.5 rounded text-xs font-medium transition ${
                                            item.gradeRec?.isPublished 
                                            ? 'text-red-200 bg-red-900/50 hover:bg-red-900/70 border border-red-900/50'
                                            : 'text-blue-200 bg-blue-900/50 hover:bg-blue-900/70 border border-blue-900/50'
                                        }`}
                                     >
      ...
      {editingCommentGradeId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <div className="bg-[#1e293b] rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-700">
                <div className="px-6 py-4 border-b border-slate-700 flex justify-between items-center bg-[#0f172a]">
                    ...
                    <button onClick={() => setEditingCommentGradeId(null)} className="text-slate-400 hover:text-white transition">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                <div className="p-6">
                    ...
                    <textarea 
                        autoFocus
                        className="w-full h-32 p-3 bg-[#0f172a] border border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-none text-white"
                        placeholder="Ej: Calificación corregida tras revisión..."
                        value={tempComment}
                        onChange={(e) => setTempComment(e.target.value)}
                    />
                </div>
                <div className="px-6 py-4 bg-[#0f172a] flex justify-end gap-3">
                    <button 
